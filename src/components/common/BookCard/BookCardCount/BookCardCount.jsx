...
import { selectCountById } from '../../../../store/cart/selectors';
import { cartSlice } from '../../../../store/cart';

export function BookCardCount({ bookId }) {
  const dispatch = useDispatch()
  const count = useSelector(selectCountById(bookId))

  return (
    <form className={styles.wrapper}>
      <button name="increase" type="button" className={styles.button} onClick={() => dispatch(cartSlice.actions.removeItem(bookId))} disabled={count === 0}>
      ...
      <button name="add" type="button" className={styles.button} onClick={() => dispatch(cartSlice.actions.addItem(bookId))}>
